...
const { stringifyBigInts: snarkjsStringify } = require('ffjavascript').utils;
...
const generateWitness = async function(cir_def, prvSeed, identity_path, quesiton, vote) {
  try {
    ...
    const wtmp = websnarkStringify.unstringifyBigInts(
      JSON.parse(JSON.stringify(snarkjsStringify.stringifyBigInts(w)))
    );
