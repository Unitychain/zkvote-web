    ...
    .then(
      (result) => {
        jsZip.loadAsync(result).then(function (zip) {
          zip.files["snark_data/proving_key.bin"].async('arraybuffer').then(fileData => component.setState({ proving_key: fileData, has_snark_data: { ...component.state.has_snark_data, proving_key: true }}))
